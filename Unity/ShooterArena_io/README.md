...
커스텀 `BaselineLogger.cs`(CSV, `--log-scenario=<label>` 플래그 시에만 활성화)로 3개 시나리오를 측정했습니다. 전체 보고서: [`PERFORMANCE_REPORT.md`](PERFORMANCE_REPORT.md)

### 시나리오 A — 대역폭 vs. 봇 수 (클라이언트 InBandwidth, SR-half)

| 봇 수 | 기준값 (AOI 20u) | 최적화 (AOI 15u) | 델타 |
|---|---|---|---|
| 0 | 0.01 KB/s | 0.02 KB/s | — (제거할 엔티티 없음) |
| 25 | 0.07 KB/s | 0.04 KB/s | **−43%** |
| 60 | 0.34 KB/s | 0.17 KB/s | **−50%** |

SendRate 스윕 (AOI-15 고정, bots=60): SR-full 0.39 → SR-half 0.17 → SR-quarter 0.16 KB/s.  
SR-half → SR-quarter 추가 단계에서 6% 절감에 그칩니다. Fusion 프로토콜 오버헤드(ACK, 제어 패킷)가 대역폭 하한값을 형성하기 때문입니다. **SR-half(30Hz)가 최적 동작점입니다.**

### 시나리오 B — GC 압력 vs. 투사체 풀링 (서버, 25봇, 300초)

| 구성 | gc_gen0/분 | avg_frame_ms |
|---|---|---|
| 풀링 OFF (기준) | 26.00 | 16.68 ms |
| 풀링 ON | 16.40 | 16.68 ms |
| 델타 | **−9.60 (−36.9%)** | 0.00 ms |

10발/초 × 25봇 = 250회/초 `Instantiate`/`Destroy` 호출. `Stack<ProjectileView>` 기반 풀이 정상 상태에서 이를 완전히 제거합니다. 프레임 타임은 변화 없음 — 효과는 GC 포즈 스파이크 감소(스무스함)이지 처리량이 아닙니다.

### 시나리오 C — 봇 AI 결정 주파수 (서버, 50봇)

| 구성 | avg_frame_ms | gc_gen0/분 | 대역폭 |
|---|---|---|---|
| AI 10Hz (기준) | 16.67 ms | 23.40 | 0.04 KB/s |
| AI 60Hz | 16.70 ms | 32.50 | 0.09 KB/s |
| 델타 | +0.03 ms (노이즈) | **+38.9%** | **+125%** |

`avg_frame_ms`는 AI 분주화로 개선되지 않습니다. `Application.targetFrameRate = 60`이 CPU 여유와 무관하게 서버를 16.67ms로 고정하기 때문입니다. **AI 분주화의 실제 효과는 CPU 절감이 아닌 GC/대역폭 안정화입니다** — A* `Path` 객체 할당 감소(GC −37%)와 `BotFireCount` 복제 빈도 감소(대역폭 −55%).
...
|---|---|---|
| AOI 반경 | 20u | **15u** |
| 투사체 풀 | OFF | **ON** (Rapid ×20 + Cannon ×5 선행 워밍) |
| 서버 송신 레이트 | SR-half (30Hz) | **SR-half (30Hz)** — 최적, 변경 없음 |
| 봇 AI 틱 간격 | 6 (10Hz) | **6 (10Hz)** — 최적, 변경 없음 |
